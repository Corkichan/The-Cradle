...
func _species(idle := 1.0, move := 0.5, rate := 1.0, threshold := 70.0) -> CreatureSpecies:
...
func _test_needs_floor_to_walk() -> void:
	...
	var rat := Creature.new(1, _species(0.2, 0.3), f, Vector2i(2, 2))
...
func _test_age_and_hunger() -> void:
	...
	for i in 400:
		real_rat.advance(TICK)
	check_approx("Maniac Sewer Rat: 0.5/s -> 10 in 20 s", real_rat.hunger, 10.0, 1e-6)
	...
	for i in 1000:
		hungry.advance(TICK)
	check("hunger caps at 100", hungry.hunger, 100.0)
	check("stays HUNGRY", hungry.state, Creature.State.HUNGRY)

	section("hungry creatures keep wandering")
	var wanderer_system := CreatureSystem.new(4, false)
	var starving: Creature = wanderer_system.spawn(_species(0.2, 0.3, 50.0), f, 1)[0]
	_run_ticks(wanderer_system, 40)
	check("already HUNGRY", starving.state, Creature.State.HUNGRY)
	var from := starving.grid_position
	var moved := false
	for i in 400:
		wanderer_system.tick(TICK)
...
func _test_invariants_over_long_run() -> void:
	...
	var rats := system.spawn(RODENT, f, 6)
	var illegal_tile := 0
	var illegal_step := 0
	var shared_tile := 0
	var steps := 0
...
func _test_own_floor_not_viewed_floor() -> void:
	...
	var upstairs := system.spawn(RODENT, floor_1, 4)
	var downstairs := system.spawn(RODENT, floor_2, 2)
